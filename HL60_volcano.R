...
setwd("~/OneDrive - Aarhus universitet/My_Lab/1_Projects/Collaborations/C_Fillat/HL-60_results/")
HL60_neg_luc <- fread(file = "topTable1_vs_2.csv", header = T)
HL60_neg_luc <- merge(HL60_neg_luc,geneid)
# plot(HL60_neg_luc$logFC,-log10(HL60_neg_luc$adj.P.Val))
# abline(h=1.30103)

HL60_luc_L142x4 <- fread(file = "topTable2_vs_4.csv", header = T)
HL60_luc_L142x4 <- merge(HL60_luc_L142x4,geneid)
# plot(HL60_luc_L142x4$logFC,-log10(HL60_luc_L142x4$adj.P.Val))
# abline(h=1.30103)

# HL60_neg_L142x4 <- fread(file = "topTable1_vs_4.csv", header = T)
# HL60_neg_L142x4 <- merge(HL60_neg_L142x4,geneid)
# plot(HL60_neg_L142x4$logFC,-log10(HL60_neg_L142x4$adj.P.Val))
# abline(h=1.30103)

HL60_luc_L142x2 <- fread(file = "topTable2_vs_3.csv", header = T)
HL60_luc_L142x2 <- merge(HL60_luc_L142x2,geneid)

HL60_luc_L142_146 <- fread(file = "topTable2_vs_5.csv", header = T)
HL60_luc_L142_146 <- merge(HL60_luc_L142_146,geneid)


library(ggplot2)
to_plot <- HL60_neg_luc
to_plot <- HL60_luc_L142x2
to_plot <- HL60_luc_L142x4
to_plot <- HL60_luc_L142_146
...
ggplot(to_plot, aes(x = logFC, y = -log10(P.Value ))) +
...
write.table(to_plot, "HL60_luc_L142_146.tsv", append = F, row.names = F, sep = "\t")

colnames(targets_142)[1] <- "gene_name"
colnames(targets_146)[1] <- "gene_name"
best_142_4X <- merge(targets_142,HL60_luc_L142x4,by="gene_name")
best_142_4X <- best_142_4X[,c(1,3,5:12,23,24)]

best_146 <- merge(targets_146,HL60_luc_L142_146,by="gene_name")
best_146 <- best_146[,c(1,3,5:12,23,24)]
best_146 <- best_146[which(best_146$`Conserved sites total`>0 & best_146$logFC>0),]

best_142_2X <- merge(targets_142,HL60_luc_L142x2,by="gene_name")
best_142_2X <- best_142_2X[,c(1,3,5:12,23,24)]

best_double <- merge(targets_142,HL60_luc_L142_146,by="gene_name")
...
write.table(best_targets, paste0("HL60_best_142","_all",".tsv"), append = F, row.names = F, sep = "\t")
write.table(best_146, paste0("HL60_best_146","_all",".tsv"), append = F, row.names = F, sep = "\t")
####
setwd("~/OneDrive - Aarhus universitet/My_Lab/1_Projects/Collaborations/C_Fillat/HL-60_results/")
best_146_HL60 <- fread(file = "HL60_best_146_all.tsv", header = T)
best_142_HL60 <- fread(file = "HL60_best_142_all.tsv", header = T)

setwd("~/OneDrive - Aarhus universitet/My_Lab/1_Projects/Collaborations/C_Fillat/THP-1_results/")
best_146_THP <- fread(file = "THP_best_146_all.tsv", header = T)
best_142_THP <- fread(file = "THP_best_142_all.tsv", header = T)

best_146_both <- best_146_THP[best_146_THP$gene_name %in% best_146_HL60$gene_name,]
rm(best_146_THP,best_146_HL60)
best_142_both <- best_142_THP[best_142_THP$gene_name %in% best_142_HL60$gene_name,]
rm(best_142_THP,best_142_HL60)

best_both_miRNA <- best_146_both[best_146_both$gene_name %in% best_142_both$gene_name,]


best_targets_both <- best_targets_THP[best_targets_THP$gene_name %in% best_targets$gene_name,]
write.table(best_targets_both, paste0("HL60_THP1_best_142","_all",".tsv"), append = F, row.names = F, sep = "\t")
